...
close all;
clc;
%% Supplemental figure 4B, from Drifting codes (2020) Wolff et al.
    ...
    for sub=1:26
        ...
        for a=1:ang_steps % loop through each orientation space
            ...
            for r=1:reps % repeat decoding "reps" times
                %% impulse 1
                % left, ccw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp1_ccw_left,bin_i_ccw_left,dat_imp1_cw_left,bin_i_cw_left);
                cued_left_dists_imp1_ccw(:,a,r)=mean(distances,2);
                
                % left, cw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp1_cw_left,bin_i_cw_left,dat_imp1_ccw_left,bin_i_ccw_left);
                cued_left_dists_imp1_cw(:,a,r)=mean(distances,2);
                
                % right, ccw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp1_ccw_right,bin_i_ccw_right,dat_imp1_cw_right,bin_i_cw_right);
                cued_right_dists_imp1_ccw(:,a,r)=mean(distances,2);
                
                % right, cw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp1_cw_right,bin_i_cw_right,dat_imp1_ccw_right,bin_i_ccw_right);
                cued_right_dists_imp1_cw(:,a,r)=mean(distances,2);
                %% impulse 2
                % left, ccw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp2_ccw_left,bin_i_ccw_left,dat_imp2_cw_left,bin_i_cw_left);
                cued_left_dists_imp2_ccw(:,a,r)=mean(distances,2);
                
                % left, cw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp2_cw_left,bin_i_cw_left,dat_imp2_ccw_left,bin_i_ccw_left);
                cued_left_dists_imp2_cw(:,a,r)=mean(distances,2);
                
                % right, ccw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp2_ccw_right,bin_i_ccw_right,dat_imp2_cw_right,bin_i_cw_right);
                cued_right_dists_imp2_ccw(:,a,r)=mean(distances,2);
                
                % right, cw
                [~,distances] = mahal_theta_basis_b_ind(dat_imp2_cw_right,bin_i_cw_right,dat_imp2_ccw_right,bin_i_ccw_right);
...
plot(angspace_ang2,ci_imp2,'Color',[0 0 0],'LineWidth',.5, 'LineStyle', ':')
line(linspace(-0,0),linspace(-0.015,0.03001),'Color',[.5 0 .5])
...
%% Supplemental Figure 4B, right (circular mean)
figure
pos=[.95 1.05];
hold all
plot(pos(1),imp1_mu_av,'o','MarkerFaceColor',[.5 0 .5],'MarkerEdgeColor','none','MarkerSize',10)
plot(pos(2),imp2_mu_av,'o','MarkerFaceColor',[.5 0 .5],'MarkerEdgeColor','none','MarkerSize',10)
plot([pos(1) pos(1)],CI_imp1,'Color',[.5 0 .5],'LineWidth',3)
plot([pos(2) pos(2)],CI_imp2,'Color',[.5 0 .5],'LineWidth',3)
plot([.93 1.07],[0 0 ],'Color','k','LineWidth',.5,'LineStyle','-')
pbaspect([1 1.5 1])
xlim([.93 1.07]); ylim([-45 45])
set(gca,'XTick',[])
...
%% Supplemental Figure 4B, right (asymmetry score)
figure
pos=[.98 1.02];
hold all
b1=boxplot([imp1_as,imp2_as],...
    'positions',pos,'Widths',0.02,'Symbol','b.');
set(findobj(gcf,'LineStyle','--'),'LineStyle','-')
set(b1(:,1),'color',[.5 0 .5]);set(b1(:,2),'color',[.5 0 .5]);
plot(pos(1),mean(imp1_as,1),'o','MarkerFaceColor',[.5 0 .5],'MarkerEdgeColor','none','MarkerSize',10)
plot(pos(2),mean(imp2_as,1),'o','MarkerFaceColor',[.5 0 .5],'MarkerEdgeColor','none','MarkerSize',10)
plot([pos(1) pos(1)],CI_imp1_as,'Color',[.5 0 .5],'LineWidth',3)
plot([pos(2) pos(2)],CI_imp2_as,'Color',[.5 0 .5],'LineWidth',3)
plot([.93 1.07],[0 0 ],'Color','k','LineWidth',.5,'LineStyle','-')
pbaspect([1 1.5 1])
xlim([.93 1.07]); 
ylim([-.012 .012])
...
%% Supplemental Figure 4B, left, insets
fhandle=figure;
set(fhandle, 'Position', [100, 100, 700, 350]);
